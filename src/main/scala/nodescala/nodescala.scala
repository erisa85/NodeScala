...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    val responseIterator = response.toIterator
    while (responseIterator.hasNext && token.nonCancelled)
      exchange.write(responseIterator.next())
    exchange.close()
  }
      ...
      Future {
        while (ct.nonCancelled) {
          ...
          nextRequest onComplete {
            case Success(r) => Future {respond(r._2, ct, handler(r._1))} //maybe a future here, not sure
