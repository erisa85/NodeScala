  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      def sequence[T](fs: List[Future[T]]): Future[List[T]] = {
        fs match {
          case Nil => Future(Nil)
          case(head::tail) => head.flatMap(headResult => sequence(tail).flatMap(tailResult => Future(headResult :: tailResult)))
        }
      }
      val futureList = sequence[T](fs)
      futureList onComplete{
        case Failure(e) => p.failure(e)
        case Success(list) =>
          if (list.forall(el => el match
          { case Failure(_) => false
            case Success(_) => true
          })) p.failure(new Throwable("At least one of the futures in the list failed"))
          else p.success(list)
      }
      p.future
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]()
      fs.foreach(el => el onComplete { p complete(_)})
      p.future
    }
...
//    }
     ...
     *  The resulting future contains a value returned by `cont`.
     */
//    def continue[S](cont: Try[T] => S): Future[S] = async{
//      await(f)
//      cont(f.value.get)
//    }
